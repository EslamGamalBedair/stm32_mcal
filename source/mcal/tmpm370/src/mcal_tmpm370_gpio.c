...
void mcal_gpio_pin_init(mcal_gpio_t *px_gpio)
{
    ...
    case MCAL_GPIO_INPUT:
        ...
        // TODO: add this
#if 0
        //config external interrupt pin
        if (INPIN_INT_CONF_x)
        {
            __disable_irq();

            CG_ClearINTReq(INPIN_GPIO[HT_INPINn].EXT_interrupt_src);
            /* Set external interrupt to wake up system */
            CG_SetSTBYReleaseINTSrc(INPIN_GPIO[HT_INPINn].EXT_interrupt_src, CG_INT_ACTIVE_STATE_RISING, ENABLE);
            NVIC_ClearPendingIRQ(INPIN_GPIO[HT_INPINn].Ext_interrupt_ID);
            NVIC_EnableIRQ(INPIN_GPIO[HT_INPINn].Ext_interrupt_ID);
            __enable_irq();
        }
#endif
